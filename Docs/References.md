# References

- [SwiftUI Guide: Triggering Actions Solely on First View Appearance - Holy Swift](https://holyswift.app/triggering-an-action-only-first-time-a-view-appears-in-swiftui/)

## Pagination

- [How to implement pagination with SwiftUI's List view](https://tanaschita.com/20230828-pagination-in-swiftui-list/)

- [How to do pagination in SwiftUI. An example of how we do it in the… | by Thomas Ricouard | Medium Engineering](https://medium.engineering/how-to-do-pagination-in-swiftui-04511be7fbd1)

## Search

- [Craft search experiences in SwiftUI - WWDC21 - Videos - Apple Developer](https://developer.apple.com/videos/play/wwdc2021/10176)

## TextField

- [Advanced SwiftUI TextField - Formatting and Validation | Fatbobman's Blog](https://fatbobman.com/en/posts/textfield-1/)

- [Advanced SwiftUI TextField - Events, Focus, Keyboard | Fatbobman's Blog](https://fatbobman.com/en/posts/textfield-event-focus-keyboard/)

## URL

- [How to open URL in Safari in SwiftUI | Sarunw](https://sarunw.com/posts/swiftui-open-url/)

## Tag Cloud

- [How to make tag flow layout using Layout protocol in SwiftUI | Swift Discovery](https://onmyway133.com/posts/how-to-make-tag-flow-layout-using-layout-protocol-in-swiftui/)

- [Implementing tags using SwiftUI - LogRocket Blog](https://blog.logrocket.com/implementing-tags-swiftui/)

- [Flow Layout / Tag Cloud in SwiftUI | Swift UI recipes](https://swiftuirecipes.com/blog/flow-layout-in-swiftui)

- [globulus/swiftui-flow-layout: Flow layout / tag cloud / collection view in SwiftUI](https://github.com/globulus/swiftui-flow-layout)

- [TagCloud/Sources/TagCloud/FlowLayoutView.swift at master · yarspirin/TagCloud](https://github.com/yarspirin/TagCloud/blob/master/Sources/TagCloud/FlowLayoutView.swift)

## SortComparator & KeyPathComparator

- [Trouble with KeyPathComparator and Bool (related: TableColumn) - Using Swift - Swift Forums](https://forums.swift.org/t/trouble-with-keypathcomparator-and-bool-related-tablecolumn/62962/4)

- [Custom Sort Comparators](https://useyourloaf.com/blog/custom-sort-comparators/)

- [Sort elements based on a property value using KeyPathComparator](https://nilcoalescing.com/blog/SortElementsBasedOnAPropertyValueUsingKeyPathComparator/)
